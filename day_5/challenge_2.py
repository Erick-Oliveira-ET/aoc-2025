...
class Cafeteria:
    ...
    def count_fresh_ingredients_id_total(self):
        count = 0

        for start, end in self.fresh_id_ranges:
            count += end - start + 1  # count theory I guess
...
assert (
    Cafeteria(*split_input_range_items(example)).count_fresh_ingredients_id_total()
    == 14
)
